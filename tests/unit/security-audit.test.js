...
// These assertions read product source off disk. path.resolve() would anchor to
// the cwd vitest was started from (tests/ per the documented workflow), landing
// on tests/src/... — resolve from this file instead so the cwd doesn't matter.
const REPO_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..", "..");
const srcPath = (...parts) => path.join(REPO_ROOT, ...parts);
...
describe("AUDIT-003: Proxy URL validation", () => {
  ...
  it("source should contain validateProxyUrl function", () => {
    const source = fs.readFileSync(
      srcPath("src/lib/network/outboundProxy.js"),
      "utf-8"
    );
...
describe("AUDIT-018: XSS escaping in OAuth callback", () => {
  it("source should contain escapeHtml function", () => {
    const source = fs.readFileSync(
      srcPath("src/lib/oauth/utils/server.js"),
      "utf-8"
    );
