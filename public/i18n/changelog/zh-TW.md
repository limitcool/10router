...
- **Qoder 國內版完整恢復 + 每日 Credits 自動領取**：qoder-cn 的 OAuth 裝置碼 / PAT 鑑權、模型目錄與用量追蹤全面可用；每日 Credits 自動領取（實驗開關 + 卡片手動領取），資源包逐包展示（各自到期日、先到期先扣）；Qoder 官方圖示與千問系即時倍率 / 夜間壢峰半價倒數計時同步上線。
- **按模型釘住上下文視窗 / 最大輸出**：模型行新增調參圖示，可對任意模型（含自動發現的）覆寫 `contextWindow` / `maxOutput`，留空即回退預設；模型清單與用量口徑同步生效。
- **超長上下文服務端自動壓縮**：請求估算超過有效視窗的 90%（閾值 80/90/95% 可選，預設開啟）時，由同一模型先把較早輪次摘要化再派發——不自壓縮的用戶端（ZCode / OpenClaw / 自建 agent）不再撞上「prompt is too long」，推理型模型同樣適配；任何失敗原樣放行，絕不因壓縮毀請求。總開關在「實驗特性」。
- **Provider 卡片拖拽排序**：卡片可拖拽重排並持久化，停用的 provider 自動置底；模型 API 清單順序跟隨卡片排序。
- **OpenCode Free 體驗渠道反濫用修復並預設開啟**；Antigravity 生圖模型補齊（gemini-3-pro-image / imagen-3.0 等）；10router-sync 外掛 v1.5.0（ZCode 套餐渠道 id 適配 + 同步鏈路加固）。

### 🛠️ 最佳化與修復

- **CodeBuddy CN 嚴格端點工具 schema 相容**：工具參數根節點為 anyOf/oneOf/allOf、`$ref`、type 陣列等形態時自動降級為純 object 根再發往上游，修復 ZCode 等用戶端整請求被 `11129` 拒絕的問題（#27）。
- **額度重置徽章不再消失**：MiMo 周報等週期性額度視窗用盡後，連線卡片仍持續顯示「還剩幾天重置」。
- **Qoder 額度解析修正**：免費帳戶每日 / 活動領取的 Credits 不再顯示 0/0。
- **CodeBuddy intl DeepSeek reasoning_effort 映射修復**（#23）。
- **穩定性移植**：4xx 請求級錯誤不再誤熔斷健康帳戶；連線測試成功自動清除陳舊退避鎖；長靜默流中斷按用戶端格式上報錯誤框，不再被誤判為正常短回覆。

## v1.1.2 (2026-09-18)

### ✨ 新增

- **用量儀表盤全新升級**：詳情頁新增生涯統計卡、GitHub 風格活躍熱力圖（日/週雙視圖、懸停明細）與節點健康度評分表（成功率/延遲/速度加權，列可展開檢視模型級明細）；「單位縮寫」開關全域生效（概覽卡與詳情頁統一，中文億/萬、其他語言 B/M/K）。
- **小米 Token Plan 出口節點智慧匹配**：新增/編輯連線時按本機網路出口自動預選 cn / sgp / ams 集群（多源容災探測，失敗靜默降級），連線測試按鈕按所選節點正確路由；MiMo 429 限流提示友好化，冷卻時間尊重上游「請約 N 秒後重試」。
- **10router-sync 外掛 v1.4.0**：新增 `/10router-sync:status` 命令——不打開儀表盤即可檢視實例狀態與今日用量。

### 🛠️ 最佳化與修復

- **CodeBuddy 11128 渠道級熔斷**：命中上游渠道風控時按渠道整體熔斷（不再逐帳號重試放大風控），回應附雙語處置提示（壓縮工作階段/稍後重試）；極端請求體積本地兜底。
- **憑證自動重新整理修復（Cline / ClinePass）**：背景定時重新整理不再 400，憑證到期自動續期恢復；失效憑證會明確標記「需重新授權」。
- **供應商與圖表治理**：失效公益站下架；OpenCode Free / MiMo Code Free 歸入「體驗」分類預設隱藏；模型家族聚合修正（gpt-6、hy4、qwen3.8 等不再被拆分）；模型類型圖例移至頂部右側，行動版自適應。
- **安全**：修復單條連線介面明文回顯小米桌面版工作階段憑證的問題。

## v1.1.1 (2026-09-14)

### ✨ 新增

- **跨帳號「配額包到期優先」全局調度策略**：解決多帳號下固定耗用主帳號導致其他帳號短期贈送包白白過期的痛點。供應商詳情頁新增獨立開關，自動優先調度即將到期的有效配額包，零阻塞、零請求延遲。
- **Command Code 用量追蹤與配額展示**：完整支援 5 小時會話滑動視窗、7 天每週限額、月度訂閱餘額與扣費展示，動態識別 Go / GOAT / Pro 等訂閱計劃，支援餘額遞減與到期倒數計時。
- **自訂模型批次啟用 / 批次停用**：相容節點與內建供應商詳情頁的模型管理支援一鍵批次開啟或收回，並修復單條更新時丟失模型配置屬性的缺陷。
- **OAuth 憑證匯入 / 匯出通用化與傳輸加密**：支援所有 OAuth 供應商憑證匯出為 AES-256-GCM 加密備份，匯入支援多級智慧查重合併，無傳輸密碼無法讀取。
- **官方品牌向量圖示升級**：整合 `@lobehub/icons`，58 個主流供應商升級為官方向量 SVG 圖示，並支援明暗主題自適應。
- **CodeBuddy 自動簽到與每日活躍會話**：簽到改為全天智慧探測與持久化備忘記錄，新增國際版每日活躍對話開關。
- **桌面客戶端增強**：功能表列全量三語本地化，新增「前往」選單（快速開啟任意網址 / 回到本地儀表板 / 最近開啟歷史管理），執行日誌支援按日期長期自動封存。

### 🐛 修復

- **用量面板 0 配額與離譜倒數計時修復**：修復當帳號額度為 0 時被錯誤渲染為無限額度 `0 / ∞` 的邏輯衝突，並過濾遠期哨兵時間戳（如公元 9999 年），不再出現幾百萬天倒數計時。
- **用量百分比計算修復**：修復金額型配額（如 Command Code $9.98）被錯誤當作百分比顯示為 10% 🔴 的問題。
- **CodeBuddy 配額展示方向最佳化**：配額包翻轉為「剩餘 / 總量」遞減顯示，與進度條方向完全一致。
- **CodeBuddy 國際版連線測試支援**：補齊專用測試端點通道標頭與身分驗證判定，不再誤報「Provider test not supported」。
- **小米桌面版 Preview 模型未登入提示友好化**：精準提示需桌面端登入，不再偽裝成限流或觸發誤冷卻。
- **新增連線後模型停用狀態即時重新整理**：連線變更時同步重新整理模型可見性，停用區模型支援直接進行連通性測試。
- **多處 i18n 與互動體驗清掃**：Endpoint 節點頁、供應商 notice 提示語與 API 金鑰按鈕全量補齊繁中與英文翻譯。

## v1.1.0 (2026-09-11)

> **版本記賬**：1.0.9 跳過未發布——原計劃的 1.0.9 內容（小米 MiMo 桌面版適配等）併入本版後規模達到 minor 級，版本號直接進位到 1.1.0；1.0.9 從未打 tag，無對應產物。
