...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.10router/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/10router/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[10router][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[10router][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[10router][runtime] chmod tray bin failed: ${e.message}`);
  }
}
