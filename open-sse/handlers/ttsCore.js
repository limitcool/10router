 ...
 */
export async function handleTtsCore({ provider, model, input, credentials, responseFormat = "mp3", language, style, voice }) {
  if (!input?.trim()) {
  ...
  try {
    ...
    const result = await synthesizeViaConfig(provider, input.trim(), model, credentials, { voice });
